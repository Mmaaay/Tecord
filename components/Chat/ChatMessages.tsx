...
import { FC, Fragment, useEffect, useState, useRef, ElementRef } from "react";
...
const ChatMessages: FC<ChatMessagesProps> = ({
  ...
  const addKey = `chat:${chatId}:${type === "channel" ? "messages" : "conversation"}:add`;
  const updateKey = `chat:${chatId}:${type === "channel" ? "messages" : "conversation"}:update`;
  ...
  if (status === "error") {
    return (
      <div ref={chatRef} className="felx-col flex flex-1 justify-center">
